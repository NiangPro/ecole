...
class FormationEzoicUnit extends Model
{
    protected $table = 'formation_ezoic_units';
    ...
    /**
     * Relation avec l'unité Ezoic
     */
    public function ezoicUnit()
    {
        return $this->belongsTo(EzoicUnit::class);
    }
    ...
    {
        $cacheKey = "formation_ezoic_ads_{$slug}" . ($position ? "_{$position}" : '');
        
        return Cache::remember($cacheKey, 3600, function () use ($slug, $position) {
            $query = static::active()
                ->with('ezoicUnit')
                ->whereHas('ezoicUnit', function($q) {
    ...
    protected static function boot()
    {
        ...
        static::saved(function ($model) {
            ...
            Cache::forget("formation_ezoic_ads_{$model->formation_slug}");
            Cache::forget("formation_ezoic_ads_{$model->formation_slug}_header");
            Cache::forget("formation_ezoic_ads_{$model->formation_slug}_content");
            Cache::forget("formation_ezoic_ads_{$model->formation_slug}_sidebar");
            Cache::forget("formation_ezoic_ads_{$model->formation_slug}_footer");
            ...
            if ($model->formation_slug === 'all') {
                ...
                foreach ($allFormations as $slug) {
                    Cache::forget("formation_ezoic_ads_{$slug}");
                    Cache::forget("formation_ezoic_ads_{$slug}_header");
                    Cache::forget("formation_ezoic_ads_{$slug}_content");
                    Cache::forget("formation_ezoic_ads_{$slug}_sidebar");
                    Cache::forget("formation_ezoic_ads_{$slug}_footer");
                }
            }
        });
