...
<style>
    * {
    ...
    .sidebar::-webkit-scrollbar-thumb {
        background: linear-gradient(180deg, #000000 0%, #2d5f8a 100%);
        border-radius: 10px;
    }
    ...
    .sidebar a.active {
        background: linear-gradient(135deg, #000000 0%, #2d5f8a 100%);
        color: white;
        font-weight: 600;
    ...
    .sidebar-toggle-btn {
        ...
        width: 60px;
        height: 60px;
        background: linear-gradient(135deg, #000000, #2d5f8a);
        border: none;
        border-radius: 50%;
        color: white;
        font-size: 24px;
...
<div class="tutorial-header">
    <div class="tutorial-header-content">
        ...
        <i class="fab fa-python" style="margin-right: 15px;"></i>
            {{ trans('app.formations.rust.title') }}
    </h1>
        <p>{{ trans('app.formations.rust.subtitle') }}</p>
        <div style="margin-top: 30px; display: flex; justify-content: center; gap: 15px; flex-wrap: wrap;">
            <button data-favorite 
                    data-favorite-type="formation" 
                    data-favorite-slug="python" 
                    data-favorite-name="{{ trans('app.formations.rust.title') }}"
...
<div class="tutorial-content">
    <div class="content-wrapper">
        ...
        <button class="sidebar-toggle-btn" id="sidebarToggle" aria-label="{{ trans('app.formations.rust.menu_open') }}">
        ...
        <aside class="sidebar" id="tutorialSidebar">
            <div style="display: flex; align-items: center; justify-content: space-between; margin-bottom: 20px; padding-bottom: 15px; border-bottom: 2px solid rgba(0, 0, 0, 0.2);">
                <h3 style="margin: 0;">{{ trans('app.formations.rust.sidebar_title') }}</h3>
                <button class="sidebar-close-btn" id="sidebarClose" style="display: none; background: none; border: none; color: #000000; font-size: 24px; cursor: pointer; padding: 5px; width: 35px; height: 35px; border-radius: 50%; transition: all 0.3s ease;" aria-label="{{ trans('app.formations.rust.menu_close') }}">
            ...
            @endphp
            @foreach(trans('app.formations.rust.sidebar_menu') as $index => $menuItem)
        ...
        <main class="main-content">
            <h1 id="intro">{{ trans('app.formations.rust.intro_title') }}</h1>
            <p>{!! trans('app.formations.rust.intro_text') !!}</p>

            <h3>{{ trans('app.formations.rust.what_is_title') }}</h3>
            <p>{!! trans('app.formations.rust.what_is_text') !!}</p>

            <div class="example-box">
                <h3 style="color: #000;">{{ trans('app.formations.rust.why_popular_title') }}</h3>
                <ol style="line-height: 2; font-size: 16px; margin-left: 20px; color: #000;">
                    @foreach(trans('app.formations.rust.why_popular_items') as $item)
                    @php
                        $parts = explode(' - ', $item);
                        $label = $parts[0] ?? '';
                        $description = $parts[1] ?? '';
            ...
            <h3>{{ trans('app.formations.rust.why_learn_title') }}</h3>
            <p>{{ trans('app.formations.rust.why_learn_text') }}</p>
            <ul style="line-height: 2; font-size: 16px; margin-left: 20px; color: #000;">
                @foreach(trans('app.formations.rust.why_learn_items') as $item)
                @php
                    $parts = explode(' - ', $item);
                    $label = $parts[0] ?? '';
                    $description = $parts[1] ?? '';
            ...
            <h3>{{ trans('app.formations.rust.prerequisites_title') }}</h3>
            <p>{{ trans('app.formations.rust.prerequisites_text') }}</p>
            <ul style="line-height: 2; font-size: 16px; margin-left: 20px; color: #000;">
                @foreach(trans('app.formations.rust.prerequisites_items') as $item)
                @php
                    $parts = explode(' - ', $item);
                    $label = $parts[0] ?? '';
                    $description = $parts[1] ?? '';
            ...
            <div class="note-box">
                <p style="color: #000;">{!! trans('app.formations.rust.prerequisites_note') !!}</p>
            </div>

            <h3>{{ trans('app.formations.rust.use_cases_title') }}</h3>
            <p>{{ trans('app.formations.rust.use_cases_text') }}</p>
            <ul style="line-height: 2; font-size: 16px; margin-left: 20px; color: #000;">
                @foreach(trans('app.formations.rust.use_cases_items') as $item)
                @php
                    $parts = explode(' - ', $item);
                    $icon = '';
                    $label = '';
            ...
            <h2 id="syntax">{{ trans('app.formations.rust.syntax_title') }}</h2>
            <p>{!! trans('app.formations.rust.syntax_text') !!}</p>
...
print(f"10 + 5 = {resultat}")</code></pre>
            ...
            <div class="example-box">
                <h3 style="color: #000;">{{ trans('app.formations.rust.syntax_points_title') }}</h3>
                <ul style="line-height: 2; font-size: 16px; margin-left: 20px; color: #000;">
                    @foreach(trans('app.formations.rust.syntax_points_items') as $item)
                    <li>{!! $item !!}</li>
                    @endforeach
                </ul>
            </div>

            <h3>{{ trans('app.formations.rust.syntax_example_title') }}</h3>
            <p>{{ trans('app.formations.rust.syntax_example_text') }}</p>
...
print(f"La moyenne est : {moyenne}")</code></pre>
            ...
            <h2 id="variables">{{ trans('app.formations.rust.variables_title') }}</h2>
            <p>{!! trans('app.formations.rust.variables_text') !!}</p>
...
CONSTANTE = 3.14159     # Convention pour les constantes</code></pre>
            ...
            <div class="example-box">
                <h3 style="color: #000;">{{ trans('app.formations.rust.variables_rules_title') }}</h3>
                <ul style="line-height: 2; font-size: 16px; margin-left: 20px; color: #000;">
                    @foreach(trans('app.formations.rust.variables_rules_items') as $item)
                    <li>{!! $item !!}</li>
                    @endforeach
                </ul>
            </div>

            <h2 id="datatypes">{{ trans('app.formations.rust.datatypes_title') }}</h2>
            <p>{{ trans('app.formations.rust.datatypes_text') }}</p>
...
prix_float = float("19.99")       # Convertir en décimal</code></pre>
            ...
            <div class="example-box">
                <h3 style="color: #000;">{{ trans('app.formations.rust.datatypes_list_title') }}</h3>
                <ul style="line-height: 2; font-size: 16px; margin-left: 20px; color: #000;">
                    @foreach(trans('app.formations.rust.datatypes_list_items') as $item)
                    @php
                        $parts = explode(' - ', $item);
                        $label = $parts[0] ?? '';
                        $description = $parts[1] ?? '';
            ...
            <h2 id="operators">{{ trans('app.formations.rust.operators_title') }}</h2>
            <p>{{ trans('app.formations.rust.operators_text') }}</p>
...
print(liste1 == liste2)    # True (valeurs égales)</code></pre>
            ...
            <h2 id="conditions">{{ trans('app.formations.rust.conditions_title') }}</h2>
            <p>{!! trans('app.formations.rust.conditions_text') !!}</p>
...
if age >= 18 and permis:
    ...
    print("Vous ne pouvez pas conduire")</code></pre>
            ...
            <h2 id="loops">{{ trans('app.formations.rust.loops_title') }}</h2>
            <p>{!! trans('app.formations.rust.loops_text') !!}</p>
...
for i in range(5):
    ...
    print("Boucle terminée")  # Exécuté si la boucle se termine normalement</code></pre>
            ...
            <h2 id="functions">{{ trans('app.formations.rust.functions_title') }}</h2>
            <p>{!! trans('app.formations.rust.functions_text') !!}</p>
...
print(carres)  # [1, 4, 9, 16, 25]</code></pre>
            ...
            <h2 id="lists">{{ trans('app.formations.rust.lists_title') }}</h2>
            <p>{{ trans('app.formations.rust.lists_text') }}</p>
...
personne.pop("ville")           # Supprimer et retourner la valeur</code></pre>
            ...
            <h2 id="modules">{{ trans('app.formations.rust.modules_title') }}</h2>
            <p>{{ trans('app.formations.rust.modules_text') }}</p>
...
# print(mon_module.ma_fonction())</code></pre>
            ...
            <h2 id="oop">{{ trans('app.formations.rust.oop_title') }}</h2>
            <p>{{ trans('app.formations.rust.oop_text') }}</p>
...
print(etudiant.etudier())       # Méthode spécifique</code></pre>
            ...
            <h2 id="files">{{ trans('app.formations.rust.files_title') }}</h2>
            <p>{!! trans('app.formations.rust.files_text') !!}</p>
...
try:
    ...
    print(f"Erreur : {e}")</code></pre>
            ...
            <div class="note-box">
                <p style="color: #000;">{!! trans('app.formations.rust.files_note') !!}</p>
            </div>

            <h2>{{ trans('app.formations.rust.next_steps_title') }}</h2>
            <p>{{ trans('app.formations.rust.next_steps_text') }}</p>
            
            <div class="example-box" style="background-color: #d4edda; border-left-color: #28a745;">
                <h3 style="color: #000;">{{ trans('app.formations.rust.next_steps_learned_title') }}</h3>
                <ul style="margin-left: 20px; line-height: 2; color: #000;">
                    @foreach(trans('app.formations.rust.next_steps_learned_items') as $item)
                    <li>{{ $item }}</li>
                    @endforeach
                </ul>
            </div>
            ...
            <div class="example-box">
                <h3 style="color: #000;">{{ trans('app.formations.rust.next_steps_further_title') }}</h3>
                <ul style="margin-left: 20px; line-height: 2; color: #000;">
                    @foreach(trans('app.formations.rust.next_steps_further_items') as $item)
                    @php
                        $parts = explode(' - ', $item);
                        $icon = '';
                        $label = '';
            ...
            <div class="nav-buttons">
                <a href="{{ route('formations.ia') }}" class="nav-btn">{{ trans('app.formations.rust.nav_previous') }}</a>
                <a href="{{ route('exercices') }}" class="nav-btn">{{ trans('app.formations.rust.nav_next') }}</a>
            </div>
        </main>
    </div>
</div>
            ...
            try {
                if (isMobile()) {
                    ...
                    if (toggleDisplay === 'none' || toggleDisplay === '') {
                        sidebarToggle.style.cssText = 'display: flex !important; visibility: visible !important; opacity: 1 !important; position: fixed !important; bottom: 20px !important; left: 20px !important; width: 60px !important; height: 60px !important; background: linear-gradient(135deg, #000000, #2d5f8a) !important; border: none !important; border-radius: 50% !important; color: white !important; font-size: 24px !important; cursor: pointer !important; z-index: 10000 !important; box-shadow: 0 8px 25px rgba(0, 0, 0, 0.4) !important; align-items: center !important; justify-content: center !important;';
                    }
