...
@section('title', trans('app.formations.dart.title') . ' | NiangProgrammeur')

@push('head')
<x-course-schema slug="dart" />
@endpush
...
@php
    $breadcrumbs = [
        ...
        ['name' => trans('app.formations.dart.title'), 'url' => url()->current()],
    ];
@endphp
...
<style>
    * {
    ...
    .tutorial-header::before {
        ...
        bottom: 0;
        background: 
            radial-gradient(circle at 20% 30%, rgba(1, 117, 194, 0.2) 0%, transparent 50%),
            radial-gradient(circle at 80% 70%, rgba(1, 117, 194, 0.15) 0%, transparent 50%);
        pointer-events: none;
    }
    ...
    .sidebar::-webkit-scrollbar-thumb {
        background: linear-gradient(180deg, #0175c2 0%, #015a9a 100%);
        border-radius: 10px;
    }
    .sidebar::-webkit-scrollbar-thumb:hover {
        background: linear-gradient(180deg, #015a9a 0%, #014070 100%);
    }
    .sidebar h3 {
        color: #0175c2;
        font-size: 20px;
        ...
        padding-bottom: 15px;
        border-bottom: 2px solid rgba(1, 117, 194, 0.2);
    }
    .sidebar a {
        display: flex;
        align-items: center;
    ...
    .sidebar a:hover {
        background: linear-gradient(135deg, rgba(1, 117, 194, 0.1) 0%, rgba(1, 117, 194, 0.05) 100%);
        color: #0175c2;
    ...
    .sidebar a.active {
        background: linear-gradient(135deg, #0175c2 0%, #015a9a 100%);
        color: white;
        font-weight: 600;
    ...
    .sidebar-close-btn {
        ...
        align-items: center;
        justify-content: center;
        background: rgba(1, 117, 194, 0.1) !important;
        border: 2px solid rgba(1, 117, 194, 0.3) !important;
        transition: all 0.3s ease;
    }
    
    .sidebar-close-btn:hover {
        background: rgba(1, 117, 194, 0.2) !important;
        border-color: rgba(1, 117, 194, 0.5) !important;
    ...
    .sidebar-toggle-btn {
        ...
        width: 60px;
        height: 60px;
        background: linear-gradient(135deg, #0175c2, #015a9a);
        border: none;
        border-radius: 50%;
        color: white;
        font-size: 24px;
        cursor: pointer;
        z-index: 10000;
        box-shadow: 0 8px 25px rgba(1, 117, 194, 0.4);
        transition: all 0.3s ease;
        align-items: center;
        justify-content: center;
    }
    ...
    .sidebar-toggle-btn.active {
        background: linear-gradient(135deg, #015a9a, #014070);
    ...
    .main-content h2 {
        ...
        margin-bottom: 15px;
        padding-top: 20px;
        border-top: 2px solid rgba(1, 117, 194, 0.2);
    }
    .main-content h3 {
        color: #000;
        font-size: 24px;
    ...
    body.dark-mode .sidebar h3 {
        color: #0175c2;
        border-bottom-color: rgba(1, 117, 194, 0.3);
    }
    ...
    body.dark-mode .sidebar a:hover {
        background: linear-gradient(135deg, rgba(1, 117, 194, 0.2) 0%, rgba(1, 117, 194, 0.1) 100%);
        color: #0175c2;
    }
...
<div class="tutorial-header">
    <div class="tutorial-header-content">
        <h1>
            <i class="fas fa-feather-alt" style="margin-right: 15px;"></i>
            {{ trans('app.formations.dart.title') }}
    </h1>
        <p>{{ trans('app.formations.dart.subtitle') }}</p>
        <div style="margin-top: 30px; display: flex; justify-content: center; gap: 15px; flex-wrap: wrap;">
            <button data-favorite 
                    data-favorite-type="formation" 
                    data-favorite-slug="dart" 
                    data-favorite-name="{{ trans('app.formations.dart.title') }}"
...
<div class="tutorial-content">
    <div class="content-wrapper">
        ...
        <button class="sidebar-toggle-btn" id="sidebarToggle" aria-label="{{ trans('app.formations.dart.menu_open') }}">
        ...
        <aside class="sidebar" id="tutorialSidebar">
            <div style="display: flex; align-items: center; justify-content: space-between; margin-bottom: 20px; padding-bottom: 15px; border-bottom: 2px solid rgba(55, 118, 171, 0.2);">
                <h3 style="margin: 0;">{{ trans('app.formations.dart.sidebar_title') }}</h3>
                <button class="sidebar-close-btn" id="sidebarClose" style="display: none; background: none; border: none; color: #0175c2; font-size: 24px; cursor: pointer; padding: 5px; width: 35px; height: 35px; border-radius: 50%; transition: all 0.3s ease;" aria-label="{{ trans('app.formations.dart.menu_close') }}">
                    <i class="fas fa-times"></i>
                </button>
            </div>
            @php
                $anchors = ['intro', 'syntax', 'variables', 'datatypes', 'operators', 'conditions', 'loops', 'functions', 'classes', 'mixins', 'futures', 'streams', 'flutter'];
            @endphp
            @foreach(trans('app.formations.dart.sidebar_menu') as $index => $menuItem)
        ...
        <main class="main-content">
            <h1 id="intro">{{ trans('app.formations.dart.intro_title') }}</h1>
            <p>{{ trans('app.formations.dart.intro_text') }}</p>

            <h3>{{ trans('app.formations.dart.what_is_title') }}</h3>
            <p>{!! trans('app.formations.dart.what_is_text') !!}</p>

            <div class="example-box">
                <h3 style="color: #000;">{{ trans('app.formations.dart.why_important_title') }}</h3>
                <ol style="line-height: 2; font-size: 16px; margin-left: 20px; color: #000;">
                    @foreach(trans('app.formations.dart.why_important_items') as $item)
                    @php
                        $parts = explode(' - ', $item);
                        $label = $parts[0] ?? '';
                        $description = $parts[1] ?? '';
            ...
            <h3>{{ trans('app.formations.dart.why_learn_title') }}</h3>
            <p>{{ trans('app.formations.dart.why_learn_text') ?? '' }}</p>
            <ul style="line-height: 2; font-size: 16px; margin-left: 20px; color: #000;">
                @foreach(trans('app.formations.dart.why_learn_items') as $item)
                @php
                    $parts = explode(' - ', $item);
                    $label = $parts[0] ?? '';
                    $description = $parts[1] ?? '';
            ...
            <h3>{{ trans('app.formations.dart.prerequisites_title') }}</h3>
            <p>{{ trans('app.formations.dart.prerequisites_text') }}</p>
            <ul style="line-height: 2; font-size: 16px; margin-left: 20px; color: #000;">
                @foreach(trans('app.formations.dart.prerequisites_items') as $item)
                @php
                    $parts = explode(' - ', $item);
                    $label = $parts[0] ?? '';
                    $description = $parts[1] ?? '';
            ...
            <div class="note-box">
                <p style="color: #000;">{!! trans('app.formations.dart.prerequisites_note') !!}</p>
            </div>

            <h3>{{ trans('app.formations.dart.use_cases_title') }}</h3>
            <p>{{ trans('app.formations.dart.use_cases_text') }}</p>
            <ul style="line-height: 2; font-size: 16px; margin-left: 20px; color: #000;">
                @foreach(trans('app.formations.dart.use_cases_items') as $item)
                @php
                    $parts = explode(' - ', $item);
                    $icon = '';
                    $label = '';
            ...
            <h2 id="syntax">{{ trans('app.formations.dart.syntax_title') }}</h2>
            <p>{!! trans('app.formations.dart.syntax_text') !!}</p>
...
printf("10 + 5 = %d\n", resultat);
            ...
            <div class="example-box">
                <h3 style="color: #000;">{{ trans('app.formations.dart.syntax_points_title') }}</h3>
                <ul style="line-height: 2; font-size: 16px; margin-left: 20px; color: #000;">
                    @foreach(trans('app.formations.dart.syntax_points_items') as $item)
                    <li>{!! $item !!}</li>
                    @endforeach
                </ul>
            </div>

            <h3>{{ trans('app.formations.dart.syntax_example_title') }}</h3>
            <p>{{ trans('app.formations.dart.syntax_example_text') }}</p>

            <div class="code-box">
                <pre><code class="language-dart">// Définition d'une fonction
...
// Fonction principale
void main() {
  List<int> notes = [15, 18, 12, 20, 16];
  double moyenne = calculerMoyenne(notes);
  print('La moyenne est : $moyenne');
}</code></pre>
            </div>

            <h2 id="variables">{{ trans('app.formations.dart.variables_title') }}</h2>
            <p>{!! trans('app.formations.dart.variables_text') !!}</p>
    ...
    return 0;
            ...
            <div class="example-box">
                ...
                <ul style="line-height: 2; font-size: 16px; margin-left: 20px; color: #000;">
                    ...
                    <li>Ne peuvent pas être des mots-clés Dart (<code>if</code>, <code>for</code>, <code>int</code>, <code>class</code>, etc.)</li>
                    <li>Convention : utilisez <code>camelCase</code> pour les variables locales, <code>PascalCase</code> pour les classes</li>
                </ul>
            </div>

            <h2 id="datatypes">{{ trans('app.formations.dart.datatypes_title') }}</h2>
            <p>{{ trans('app.formations.dart.datatypes_text') }}</p>
...
int main() {
    ...
    return 0;
            ...
            <div class="example-box">
                <h3 style="color: #000;">{{ trans('app.formations.dart.datatypes_list_title') }}</h3>
                <ul style="line-height: 2; font-size: 16px; margin-left: 20px; color: #000;">
                    @foreach(trans('app.formations.dart.datatypes_list_items') as $item)
                    <li>{!! $item !!}</li>
                    @endforeach
                </ul>
            </div>

            <h2 id="operators">{{ trans('app.formations.dart.operators_title') }}</h2>
            <p>{{ trans('app.formations.dart.operators_text') }}</p>
...
int main() {
    ...
    return 0;
            ...
            <h2 id="conditions">{{ trans('app.formations.dart.conditions_title') }}</h2>
            <p>{!! trans('app.formations.dart.conditions_text') !!}</p>
...
if age >= 18 and permis:
    ...
    print("Vous ne pouvez pas conduire")</code></pre>
            ...
            <h2 id="loops">{{ trans('app.formations.dart.loops_title') }}</h2>
            <p>{!! trans('app.formations.dart.loops_text') !!}</p>
...
for i in range(5):
    ...
    print("Boucle terminée")  # Exécuté si la boucle se termine normalement</code></pre>
            ...
            <h2 id="functions">{{ trans('app.formations.dart.functions_title') }}</h2>
            <p>{!! trans('app.formations.dart.functions_text') !!}</p>
...
print(carres)  # [1, 4, 9, 16, 25]</code></pre>
            ...
            <h2 id="classes">{{ trans('app.formations.dart.classes_title') }}</h2>
            <p>{{ trans('app.formations.dart.classes_text') }}</p>

            <div class="code-box">
                <pre><code class="language-dart">// Définition d'une classe
class Personne {
  // Propriétés
  String nom;
  int age;
...
void main() {
  ...
  p2.afficher();
            ...
            <h2 id="mixins">{{ trans('app.formations.dart.mixins_title') }}</h2>
            <p>{{ trans('app.formations.dart.mixins_text') }}</p>

            <div class="code-box">
                <pre><code class="language-dart">// Définition d'un mixin
mixin Voler {
  void voler() {
    print('Je vole !');
  }
...
void main() {
  ...
  canard.nager();
            ...
            <h2 id="futures">{{ trans('app.formations.dart.futures_title') }}</h2>
            <p>{{ trans('app.formations.dart.futures_text') }}</p>

            <div class="code-box">
                <pre><code class="language-dart">import 'dart:async';

// Fonction asynchrone
Future<String> téléchargerDonnées() async {
...
void main() async {
  ...
  téléchargerDonnées().then((données) {
    print('Avec then(): $données');
            ...
            <h2 id="streams">{{ trans('app.formations.dart.streams_title') }}</h2>
            <p>{{ trans('app.formations.dart.streams_text') }}</p>
...
void main() async {
  await for (int nombre in compter()) {
    print('Nombre: $nombre');
            ...
            <h2 id="flutter">{{ trans('app.formations.dart.flutter_title') }}</h2>
            <p>{{ trans('app.formations.dart.flutter_text') }}</p>
      ...
      home: Scaffold(
        ...
        body: Center(
          child: Text('Hello Flutter!'),
            ...
            <div class="note-box">
                <p style="color: #000;"><strong>💡 Note importante :</strong> Dart utilise un garbage collector (GC) pour gérer automatiquement la mémoire. Vous n'avez pas besoin de libérer manuellement la mémoire comme en C/C++. Le GC s'occupe de libérer les objets non utilisés automatiquement.</p>
            </div>

            <h2>{{ trans('app.formations.dart.next_steps_title') }}</h2>
            <p>{{ trans('app.formations.dart.next_steps_text') }}</p>
            
            <div class="example-box" style="background-color: #d4edda; border-left-color: #28a745;">
                <h3 style="color: #000;">{{ trans('app.formations.dart.next_steps_learned_title') }}</h3>
                <ul style="margin-left: 20px; line-height: 2; color: #000;">
                    @foreach(trans('app.formations.dart.next_steps_learned_items') as $item)
                    <li>{{ $item }}</li>
                    @endforeach
                </ul>
            </div>
            ...
            <div class="example-box">
                <h3 style="color: #000;">{{ trans('app.formations.dart.next_steps_further_title') }}</h3>
                <ul style="margin-left: 20px; line-height: 2; color: #000;">
                    @foreach(trans('app.formations.dart.next_steps_further_items') as $item)
                    @php
                        $parts = explode(' - ', $item);
                        $icon = '';
                        $label = '';
            ...
            <div class="nav-buttons">
                <a href="{{ route('formations.csharp') }}" class="nav-btn">{{ trans('app.formations.dart.nav_previous') }}</a>
                <a href="{{ route('exercices') }}" class="nav-btn">{{ trans('app.formations.dart.nav_next') }}</a>
            </div>
        </main>
    </div>
</div>
...
<script>
    ...
    @auth
    (function() {
        const formationSlug = 'dart';
        const sections = [
            'intro', 'syntax', 'variables', 'datatypes', 'operators', 'conditions', 
            'loops', 'functions', 'classes', 'mixins', 'futures', 'streams', 'flutter'
        ];
            ...
            notification.innerHTML = `
                <div style="display: flex; align-items: center; gap: 1rem;">
                    ...
                    <div>
                        ...
                        <p style="margin: 0; font-size: 0.9rem; opacity: 0.9;">Félicitations ! Vous avez terminé la formation Dart.</p>
                    </div>
                </div>
            `;
