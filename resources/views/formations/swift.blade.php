...
@section('title', trans('app.formations.swift.title') . ' | NiangProgrammeur')

@push('head')
<x-course-schema slug="swift" />
@endpush
...
@php
    $breadcrumbs = [
        ...
        ['name' => trans('app.formations.swift.title'), 'url' => url()->current()],
    ];
@endphp
...
<style>
    * {
    ...
    .sidebar::-webkit-scrollbar-thumb {
        background: linear-gradient(180deg, #fa7343 0%, #c85a2f 100%);
        border-radius: 10px;
    }
    .sidebar::-webkit-scrollbar-thumb:hover {
        background: linear-gradient(180deg, #c85a2f 0%, #a04825 100%);
    }
    .sidebar h3 {
        color: #fa7343;
        font-size: 20px;
    ...
    .sidebar a.active {
        background: linear-gradient(135deg, #fa7343 0%, #c85a2f 100%);
        color: white;
        font-weight: 600;
    ...
    .sidebar-toggle-btn {
        ...
        width: 60px;
        height: 60px;
        background: linear-gradient(135deg, #fa7343, #c85a2f);
        border: none;
        border-radius: 50%;
        color: white;
        font-size: 24px;
    ...
    .sidebar-toggle-btn.active {
        background: linear-gradient(135deg, #c85a2f, #a04825);
    ...
    /* Bouton de copie - Même taille que le label swift */
    .copy-code-btn {
        position: absolute;
        top: 10px;
        right: 100px;
...
<div class="tutorial-header">
    <div class="tutorial-header-content">
        <h1>
        <i class="fab fa-swift" style="margin-right: 15px;"></i>
            {{ trans('app.formations.swift.title') }}
    </h1>
        <p>{{ trans('app.formations.swift.subtitle') }}</p>
        <div style="margin-top: 30px; display: flex; justify-content: center; gap: 15px; flex-wrap: wrap;">
            <button data-favorite 
                    data-favorite-type="formation" 
                    data-favorite-slug="swift" 
                    data-favorite-name="{{ trans('app.formations.swift.title') }}"
...
<div class="tutorial-content">
    <div class="content-wrapper">
        ...
        <button class="sidebar-toggle-btn" id="sidebarToggle" aria-label="{{ trans('app.formations.swift.menu_open') }}">
        ...
        <aside class="sidebar" id="tutorialSidebar">
            <div style="display: flex; align-items: center; justify-content: space-between; margin-bottom: 20px; padding-bottom: 15px; border-bottom: 2px solid rgba(55, 118, 171, 0.2);">
                <h3 style="margin: 0;">{{ trans('app.formations.swift.sidebar_title') }}</h3>
                <button class="sidebar-close-btn" id="sidebarClose" style="display: none; background: none; border: none; color: #fa7343; font-size: 24px; cursor: pointer; padding: 5px; width: 35px; height: 35px; border-radius: 50%; transition: all 0.3s ease;" aria-label="{{ trans('app.formations.swift.menu_close') }}">
            ...
            @endphp
            @foreach(trans('app.formations.swift.sidebar_menu') as $index => $menuItem)
        ...
        <main class="main-content">
            <h1 id="intro">{{ trans('app.formations.swift.intro_title') }}</h1>
            <p>{!! trans('app.formations.swift.intro_text') !!}</p>

            <h3>{{ trans('app.formations.swift.what_is_title') }}</h3>
            <p>{!! trans('app.formations.swift.what_is_text') !!}</p>

            <div class="example-box">
                <h3 style="color: #000;">{{ trans('app.formations.swift.why_popular_title') }}</h3>
                <ol style="line-height: 2; font-size: 16px; margin-left: 20px; color: #000;">
                    @foreach(trans('app.formations.swift.why_popular_items') as $item)
                    @php
                        $parts = explode(' - ', $item);
                        $label = $parts[0] ?? '';
                        $description = $parts[1] ?? '';
            ...
            <h3>{{ trans('app.formations.swift.why_learn_title') }}</h3>
            <p>{{ trans('app.formations.swift.why_learn_text') }}</p>
            <ul style="line-height: 2; font-size: 16px; margin-left: 20px; color: #000;">
                @foreach(trans('app.formations.swift.why_learn_items') as $item)
                @php
                    $parts = explode(' - ', $item);
                    $label = $parts[0] ?? '';
                    $description = $parts[1] ?? '';
            ...
            <h3>{{ trans('app.formations.swift.prerequisites_title') }}</h3>
            <p>{{ trans('app.formations.swift.prerequisites_text') }}</p>
            <ul style="line-height: 2; font-size: 16px; margin-left: 20px; color: #000;">
                @foreach(trans('app.formations.swift.prerequisites_items') as $item)
                @php
                    $parts = explode(' - ', $item);
                    $label = $parts[0] ?? '';
                    $description = $parts[1] ?? '';
            ...
            <div class="note-box">
                <p style="color: #000;">{!! trans('app.formations.swift.prerequisites_note') !!}</p>
            </div>

            <h3>{{ trans('app.formations.swift.use_cases_title') }}</h3>
            <p>{{ trans('app.formations.swift.use_cases_text') }}</p>
            <ul style="line-height: 2; font-size: 16px; margin-left: 20px; color: #000;">
                @foreach(trans('app.formations.swift.use_cases_items') as $item)
                @php
                    $parts = explode(' - ', $item);
                    $icon = '';
                    $label = '';
            ...
            <h2 id="syntax">{{ trans('app.formations.swift.syntax_title') }}</h2>
            <p>{!! trans('app.formations.swift.syntax_text') !!}</p>

            <div class="code-box">
                <pre><code class="language-swift">// Premier programme Swift
print("Bonjour, monde !")
...
// Opérations simples
let resultat = 10 + 5
print("10 + 5 = \(resultat)")</code></pre>
            </div>

            <div class="example-box">
                <h3 style="color: #000;">{{ trans('app.formations.swift.syntax_points_title') }}</h3>
                <ul style="line-height: 2; font-size: 16px; margin-left: 20px; color: #000;">
                    @foreach(trans('app.formations.swift.syntax_points_items') as $item)
                    <li>{!! $item !!}</li>
                    @endforeach
                </ul>
            </div>

            <h3>{{ trans('app.formations.swift.syntax_example_title') }}</h3>
            <p>{{ trans('app.formations.swift.syntax_example_text') }}</p>

            <div class="code-box">
                <pre><code class="language-swift">// Définition d'une fonction
...
print("La moyenne est : \(moyenne)")</code></pre>
            </div>

            <h2 id="variables">{{ trans('app.formations.swift.variables_title') }}</h2>
            <p>{!! trans('app.formations.swift.variables_text') !!}</p>

            <div class="code-box">
                <pre><code class="language-swift">// Déclaration de variables et constantes
...
let CONSTANTE = 3.14159         // Convention pour les constantes</code></pre>
            </div>

            <div class="example-box">
                <h3 style="color: #000;">{{ trans('app.formations.swift.variables_rules_title') }}</h3>
                <ul style="line-height: 2; font-size: 16px; margin-left: 20px; color: #000;">
                    @foreach(trans('app.formations.swift.variables_rules_items') as $item)
                    <li>{!! $item !!}</li>
                    @endforeach
                </ul>
            </div>

            <h2 id="datatypes">{{ trans('app.formations.swift.datatypes_title') }}</h2>
            <p>{{ trans('app.formations.swift.datatypes_text') }}</p>

            <div class="code-box">
                <pre><code class="language-swift">// Types de base
...
let prixDouble = Double("19.99")      // Convertir en Double (Optional)</code></pre>
            </div>

            <div class="example-box">
                <h3 style="color: #000;">{{ trans('app.formations.swift.datatypes_list_title') }}</h3>
                <ul style="line-height: 2; font-size: 16px; margin-left: 20px; color: #000;">
                    @foreach(trans('app.formations.swift.datatypes_list_items') as $item)
                    @php
                        $parts = explode(' - ', $item);
                        $label = $parts[0] ?? '';
                        $description = $parts[1] ?? '';
            ...
            <h2 id="operators">{{ trans('app.formations.swift.operators_title') }}</h2>
            <p>{{ trans('app.formations.swift.operators_text') }}</p>

            <div class="code-box">
                <pre><code class="language-swift">// Opérateurs arithmétiques
let a = 10
let b = 3
...
print(liste1 == liste2)     // true (valeurs égales)</code></pre>
            ...
            <h2 id="conditions">{{ trans('app.formations.swift.conditions_title') }}</h2>
            <p>{!! trans('app.formations.swift.conditions_text') !!}</p>

            <div class="code-box">
                <pre><code class="language-swift">// Structure if simple
let age = 20
...
if age4 >= 18 && permis {
    ...
    print("Vous ne pouvez pas conduire")
            ...
            <h2 id="loops">{{ trans('app.formations.swift.loops_title') }}</h2>
            <p>{!! trans('app.formations.swift.loops_text') !!}</p>

            <div class="code-box">
                <pre><code class="language-swift">// Boucle for-in avec range
for i in 0..<5 {
...
repeat {
    ...
    compteur3 += 1
            ...
            <h2 id="functions">{{ trans('app.formations.swift.functions_title') }}</h2>
            <p>{!! trans('app.formations.swift.functions_text') !!}</p>

            <div class="code-box">
                <pre><code class="language-swift">// Fonction simple (sans paramètres)
func direBonjour() {
    print("Bonjour !")
}
...
print(carres)  // [1, 4, 9, 16, 25]</code></pre>
            ...
            <h2 id="lists">{{ trans('app.formations.swift.lists_title') }}</h2>
            <p>{{ trans('app.formations.swift.lists_text') }}</p>

            <div class="code-box">
                <pre><code class="language-swift">// ========== ARRAYS ==========
...
personne.pop("ville")           # Supprimer et retourner la valeur</code></pre>
            ...
            <h2 id="modules">{{ trans('app.formations.swift.modules_title') }}</h2>
            <p>{{ trans('app.formations.swift.modules_text') }}</p>

            <div class="code-box">
                <pre><code class="language-swift"># Importer un module complet
import math
...
# print(mon_module.ma_fonction())</code></pre>
            ...
            <h2 id="oop">{{ trans('app.formations.swift.oop_title') }}</h2>
            <p>{{ trans('app.formations.swift.oop_text') }}</p>

            <div class="code-box">
                <pre><code class="language-swift"># Définir une classe
class Personne:
...
print(etudiant.etudier())       # Méthode spécifique</code></pre>
            ...
            <h2 id="files">{{ trans('app.formations.swift.files_title') }}</h2>
            <p>{!! trans('app.formations.swift.files_text') !!}</p>

            <div class="code-box">
                <pre><code class="language-swift"># Écrire dans un fichier (mode 'w' = write)
...
try:
    ...
    print(f"Erreur : {e}")</code></pre>
            ...
            <div class="note-box">
                <p style="color: #000;">{!! trans('app.formations.swift.files_note') !!}</p>
            </div>

            <h2>{{ trans('app.formations.swift.next_steps_title') }}</h2>
            <p>{{ trans('app.formations.swift.next_steps_text') }}</p>
            
            <div class="example-box" style="background-color: #d4edda; border-left-color: #28a745;">
                <h3 style="color: #000;">{{ trans('app.formations.swift.next_steps_learned_title') }}</h3>
                <ul style="margin-left: 20px; line-height: 2; color: #000;">
                    @foreach(trans('app.formations.swift.next_steps_learned_items') as $item)
                    <li>{{ $item }}</li>
                    @endforeach
                </ul>
            </div>
            ...
            <div class="example-box">
                <h3 style="color: #000;">{{ trans('app.formations.swift.next_steps_further_title') }}</h3>
                <ul style="margin-left: 20px; line-height: 2; color: #000;">
                    @foreach(trans('app.formations.swift.next_steps_further_items') as $item)
                    @php
                        $parts = explode(' - ', $item);
                        $icon = '';
                        $label = '';
            ...
            <div class="nav-buttons">
                <a href="{{ route('formations.ruby') }}" class="nav-btn">{{ trans('app.formations.swift.nav_previous') }}</a>
                <a href="{{ route('exercices') }}" class="nav-btn">{{ trans('app.formations.swift.nav_next') }}</a>
            </div>
        </main>
    </div>
</div>
            ...
            try {
                if (isMobile()) {
                    ...
                    if (toggleDisplay === 'none' || toggleDisplay === '') {
                        sidebarToggle.style.cssText = 'display: flex !important; visibility: visible !important; opacity: 1 !important; position: fixed !important; bottom: 20px !important; left: 20px !important; width: 60px !important; height: 60px !important; background: linear-gradient(135deg, #fa7343, #c85a2f) !important; border: none !important; border-radius: 50% !important; color: white !important; font-size: 24px !important; cursor: pointer !important; z-index: 10000 !important; box-shadow: 0 8px 25px rgba(55, 118, 171, 0.4) !important; align-items: center !important; justify-content: center !important;';
                    }
            ...
            notification.innerHTML = `
                <div style="display: flex; align-items: center; gap: 1rem;">
                    ...
                    <div>
                        ...
                        <p style="margin: 0; font-size: 0.9rem; opacity: 0.9;">Félicitations ! Vous avez terminé la formation swift.</p>
                    </div>
                </div>
            `;
