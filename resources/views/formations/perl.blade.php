...
<style>
    * {
    ...
    .sidebar::-webkit-scrollbar-thumb {
        background: linear-gradient(180deg, #39457e 0%, #2d3558 100%);
        border-radius: 10px;
    }
    .sidebar::-webkit-scrollbar-thumb:hover {
        background: linear-gradient(180deg, #2d3558 0%, #1f2842 100%);
    }
    .sidebar h3 {
        color: #39457e;
        font-size: 20px;
    ...
    .sidebar a.active {
        background: linear-gradient(135deg, #39457e 0%, #2d3558 100%);
        color: white;
        font-weight: 600;
    ...
    .sidebar-toggle-btn {
        ...
        width: 60px;
        height: 60px;
        background: linear-gradient(135deg, #39457e, #2d3558);
        border: none;
        border-radius: 50%;
        color: white;
        font-size: 24px;
    ...
    .sidebar-toggle-btn.active {
        background: linear-gradient(135deg, #2d3558, #1f2842);
    ...
    /* Bouton de copie - Même taille que le label perl */
    .copy-code-btn {
        position: absolute;
        top: 10px;
        right: 100px;
...
<div class="tutorial-header">
    <div class="tutorial-header-content">
        ...
        <i class="fas fa-code" style="margin-right: 15px;"></i>
            {{ trans('app.formations.perl.title') }}
    </h1>
        <p>{{ trans('app.formations.perl.subtitle') }}</p>
        <div style="margin-top: 30px; display: flex; justify-content: center; gap: 15px; flex-wrap: wrap;">
            <button data-favorite 
                    data-favorite-type="formation" 
                    data-favorite-slug="perl" 
                    data-favorite-name="{{ trans('app.formations.perl.title') }}"
...
<div class="tutorial-content">
    <div class="content-wrapper">
        ...
        <button class="sidebar-toggle-btn" id="sidebarToggle" aria-label="{{ trans('app.formations.perl.menu_open') }}">
        ...
        <aside class="sidebar" id="tutorialSidebar">
            <div style="display: flex; align-items: center; justify-content: space-between; margin-bottom: 20px; padding-bottom: 15px; border-bottom: 2px solid rgba(55, 118, 171, 0.2);">
                <h3 style="margin: 0;">{{ trans('app.formations.perl.sidebar_title') }}</h3>
                <button class="sidebar-close-btn" id="sidebarClose" style="display: none; background: none; border: none; color: #39457e; font-size: 24px; cursor: pointer; padding: 5px; width: 35px; height: 35px; border-radius: 50%; transition: all 0.3s ease;" aria-label="{{ trans('app.formations.perl.menu_close') }}">
            ...
            @endphp
            @foreach(trans('app.formations.perl.sidebar_menu') as $index => $menuItem)
        ...
        <main class="main-content">
            <h1 id="intro">{{ trans('app.formations.perl.intro_title') }}</h1>
            <p>{!! trans('app.formations.perl.intro_text') !!}</p>

            <h3>{{ trans('app.formations.perl.what_is_title') }}</h3>
            <p>{!! trans('app.formations.perl.what_is_text') !!}</p>

            <div class="example-box">
                <h3 style="color: #000;">{{ trans('app.formations.perl.why_popular_title') }}</h3>
                <ol style="line-height: 2; font-size: 16px; margin-left: 20px; color: #000;">
                    @foreach(trans('app.formations.perl.why_popular_items') as $item)
                    @php
                        $parts = explode(' - ', $item);
                        $label = $parts[0] ?? '';
                        $description = $parts[1] ?? '';
            ...
            <h3>{{ trans('app.formations.perl.why_learn_title') }}</h3>
            <p>{{ trans('app.formations.perl.why_learn_text') }}</p>
            <ul style="line-height: 2; font-size: 16px; margin-left: 20px; color: #000;">
                @foreach(trans('app.formations.perl.why_learn_items') as $item)
                @php
                    $parts = explode(' - ', $item);
                    $label = $parts[0] ?? '';
                    $description = $parts[1] ?? '';
            ...
            <h3>{{ trans('app.formations.perl.prerequisites_title') }}</h3>
            <p>{{ trans('app.formations.perl.prerequisites_text') }}</p>
            <ul style="line-height: 2; font-size: 16px; margin-left: 20px; color: #000;">
                @foreach(trans('app.formations.perl.prerequisites_items') as $item)
                @php
                    $parts = explode(' - ', $item);
                    $label = $parts[0] ?? '';
                    $description = $parts[1] ?? '';
            ...
            <div class="note-box">
                <p style="color: #000;">{!! trans('app.formations.perl.prerequisites_note') !!}</p>
            </div>

            <h3>{{ trans('app.formations.perl.use_cases_title') }}</h3>
            <p>{{ trans('app.formations.perl.use_cases_text') }}</p>
            <ul style="line-height: 2; font-size: 16px; margin-left: 20px; color: #000;">
                @foreach(trans('app.formations.perl.use_cases_items') as $item)
                @php
                    $parts = explode(' - ', $item);
                    $icon = '';
                    $label = '';
            ...
            <h2 id="syntax">{{ trans('app.formations.perl.syntax_title') }}</h2>
            <p>{!! trans('app.formations.perl.syntax_text') !!}</p>
...
print "10 + 5 = $resultat\n";</code></pre>
            ...
            <div class="example-box">
                <h3 style="color: #000;">{{ trans('app.formations.perl.syntax_points_title') }}</h3>
                <ul style="line-height: 2; font-size: 16px; margin-left: 20px; color: #000;">
                    @foreach(trans('app.formations.perl.syntax_points_items') as $item)
                    <li>{!! $item !!}</li>
                    @endforeach
                </ul>
            </div>

            <h3>{{ trans('app.formations.perl.syntax_example_title') }}</h3>
            <p>{{ trans('app.formations.perl.syntax_example_text') }}</p>

            <div class="code-box">
                <pre><code class="language-perl"># Définition d'une fonction (subroutine)
...
print "La moyenne est : $moyenne\n";</code></pre>
            ...
            <h2 id="variables">{{ trans('app.formations.perl.variables_title') }}</h2>
            <p>{!! trans('app.formations.perl.variables_text') !!}</p>

            <div class="code-box">
                <pre><code class="language-perl"># Déclaration de variables scalaires ($)
...
use constant CONSTANTE => 3.14159;  # Constante</code></pre>
            </div>

            <div class="example-box">
                <h3 style="color: #000;">{{ trans('app.formations.perl.variables_rules_title') }}</h3>
                <ul style="line-height: 2; font-size: 16px; margin-left: 20px; color: #000;">
                    @foreach(trans('app.formations.perl.variables_rules_items') as $item)
                    <li>{!! $item !!}</li>
                    @endforeach
                </ul>
            </div>

            <h2 id="datatypes">{{ trans('app.formations.perl.datatypes_title') }}</h2>
            <p>{{ trans('app.formations.perl.datatypes_text') }}</p>

            <div class="code-box">
                <pre><code class="language-perl"># Types de base (scalaires avec $)
...
my $prix_float = "19.99" + 0;      # Convertir en décimal</code></pre>
            </div>

            <div class="example-box">
                <h3 style="color: #000;">{{ trans('app.formations.perl.datatypes_list_title') }}</h3>
                <ul style="line-height: 2; font-size: 16px; margin-left: 20px; color: #000;">
                    @foreach(trans('app.formations.perl.datatypes_list_items') as $item)
                    @php
                        $parts = explode(' - ', $item);
                        $label = $parts[0] ?? '';
                        $description = $parts[1] ?? '';
            ...
            <h2 id="operators">{{ trans('app.formations.perl.operators_title') }}</h2>
            <p>{{ trans('app.formations.perl.operators_text') }}</p>

            <div class="code-box">
                <pre><code class="language-perl"># Opérateurs arithmétiques
a = 10
b = 3
...
print(liste1 == liste2)    # True (valeurs égales)</code></pre>
            ...
            <h2 id="conditions">{{ trans('app.formations.perl.conditions_title') }}</h2>
            <p>{!! trans('app.formations.perl.conditions_text') !!}</p>

            <div class="code-box">
                <pre><code class="language-perl"># Structure if simple
age = 20
...
if age >= 18 and permis:
    ...
    print("Vous ne pouvez pas conduire")</code></pre>
            ...
            <h2 id="loops">{{ trans('app.formations.perl.loops_title') }}</h2>
            <p>{!! trans('app.formations.perl.loops_text') !!}</p>

            <div class="code-box">
                <pre><code class="language-perl"># Boucle for avec range()
for i in range(5):
...
for i in range(5):
    ...
    print("Boucle terminée")  # Exécuté si la boucle se termine normalement</code></pre>
            ...
            <h2 id="functions">{{ trans('app.formations.perl.functions_title') }}</h2>
            <p>{!! trans('app.formations.perl.functions_text') !!}</p>

            <div class="code-box">
                <pre><code class="language-perl"># Fonction simple (sans paramètres)
def dire_bonjour():
    print("Bonjour !")
...
print(carres)  # [1, 4, 9, 16, 25]</code></pre>
            ...
            <h2 id="lists">{{ trans('app.formations.perl.lists_title') }}</h2>
            <p>{{ trans('app.formations.perl.lists_text') }}</p>

            <div class="code-box">
                <pre><code class="language-perl"># ========== LISTES ==========
# Création de listes
nombres = [1, 2, 3, 4, 5]
fruits = ["pomme", "banane", "orange"]
...
personne.pop("ville")           # Supprimer et retourner la valeur</code></pre>
            ...
            <h2 id="modules">{{ trans('app.formations.perl.modules_title') }}</h2>
            <p>{{ trans('app.formations.perl.modules_text') }}</p>

            <div class="code-box">
                <pre><code class="language-perl"># Importer un module complet
import math
...
# print(mon_module.ma_fonction())</code></pre>
            ...
            <h2 id="oop">{{ trans('app.formations.perl.oop_title') }}</h2>
            <p>{{ trans('app.formations.perl.oop_text') }}</p>

            <div class="code-box">
                <pre><code class="language-perl"># Définir une classe
class Personne:
...
print(etudiant.etudier())       # Méthode spécifique</code></pre>
            ...
            <h2 id="files">{{ trans('app.formations.perl.files_title') }}</h2>
            <p>{!! trans('app.formations.perl.files_text') !!}</p>

            <div class="code-box">
                <pre><code class="language-perl"># Écrire dans un fichier (mode 'w' = write)
...
try:
    ...
    print(f"Erreur : {e}")</code></pre>
            ...
            <div class="note-box">
                <p style="color: #000;">{!! trans('app.formations.perl.files_note') !!}</p>
            </div>

            <h2>{{ trans('app.formations.perl.next_steps_title') }}</h2>
            <p>{{ trans('app.formations.perl.next_steps_text') }}</p>
            
            <div class="example-box" style="background-color: #d4edda; border-left-color: #28a745;">
                <h3 style="color: #000;">{{ trans('app.formations.perl.next_steps_learned_title') }}</h3>
                <ul style="margin-left: 20px; line-height: 2; color: #000;">
                    @foreach(trans('app.formations.perl.next_steps_learned_items') as $item)
                    <li>{{ $item }}</li>
                    @endforeach
                </ul>
            </div>
            ...
            <div class="example-box">
                <h3 style="color: #000;">{{ trans('app.formations.perl.next_steps_further_title') }}</h3>
                <ul style="margin-left: 20px; line-height: 2; color: #000;">
                    @foreach(trans('app.formations.perl.next_steps_further_items') as $item)
                    @php
                        $parts = explode(' - ', $item);
                        $icon = '';
                        $label = '';
            ...
            <div class="nav-buttons">
                <a href="{{ route('formations.swift') }}" class="nav-btn">{{ trans('app.formations.perl.nav_previous') }}</a>
                <a href="{{ route('exercices') }}" class="nav-btn">{{ trans('app.formations.perl.nav_next') }}</a>
            </div>
        </main>
    </div>
</div>
            ...
            try {
                if (isMobile()) {
                    ...
                    if (toggleDisplay === 'none' || toggleDisplay === '') {
                        sidebarToggle.style.cssText = 'display: flex !important; visibility: visible !important; opacity: 1 !important; position: fixed !important; bottom: 20px !important; left: 20px !important; width: 60px !important; height: 60px !important; background: linear-gradient(135deg, #39457e, #2d3558) !important; border: none !important; border-radius: 50% !important; color: white !important; font-size: 24px !important; cursor: pointer !important; z-index: 10000 !important; box-shadow: 0 8px 25px rgba(55, 118, 171, 0.4) !important; align-items: center !important; justify-content: center !important;';
                    }
            ...
            notification.innerHTML = `
                <div style="display: flex; align-items: center; gap: 1rem;">
                    ...
                    <div>
                        ...
                        <p style="margin: 0; font-size: 0.9rem; opacity: 0.9;">Félicitations ! Vous avez terminé la formation perl.</p>
                    </div>
                </div>
            `;
