...
@section('title', trans('app.formations.go.title') . ' | NiangProgrammeur')

@push('head')
<x-course-schema slug="go" />
@endpush
...
@php
    $breadcrumbs = [
        ...
        ['name' => trans('app.formations.go.title'), 'url' => url()->current()],
    ];
@endphp
...
<style>
    * {
    ...
    .tutorial-header::before {
        ...
        bottom: 0;
        background: 
            radial-gradient(circle at 20% 30%, rgba(0, 173, 216, 0.2) 0%, transparent 50%),
            radial-gradient(circle at 80% 70%, rgba(0, 173, 216, 0.15) 0%, transparent 50%);
        pointer-events: none;
    }
    ...
    .sidebar::-webkit-scrollbar-thumb {
        background: linear-gradient(180deg, #00add8 0%, #2d5f8a 100%);
        border-radius: 10px;
    }
    ...
    .sidebar h3 {
        ...
        padding-bottom: 15px;
        border-bottom: 2px solid rgba(0, 173, 216, 0.2);
    }
    .sidebar a {
        display: flex;
        align-items: center;
    ...
    .sidebar a:hover {
        background: linear-gradient(135deg, rgba(0, 173, 216, 0.1) 0%, rgba(0, 173, 216, 0.05) 100%);
        color: #00add8;
    ...
    .sidebar a.active {
        background: linear-gradient(135deg, #00add8 0%, #2d5f8a 100%);
        color: white;
        font-weight: 600;
    ...
    .sidebar-close-btn {
        ...
        border: 2px solid rgba(0, 173, 216, 0.3) !important;
        transition: all 0.3s ease;
    }
    
    .sidebar-close-btn:hover {
        background: rgba(0, 173, 216, 0.2) !important;
    ...
    .sidebar-toggle-btn {
        ...
        width: 60px;
        height: 60px;
        background: linear-gradient(135deg, #00add8, #2d5f8a);
        border: none;
        border-radius: 50%;
        color: white;
        font-size: 24px;
    ...
    .sidebar-toggle-btn:hover {
        transform: scale(1.1);
        box-shadow: 0 12px 35px rgba(0, 173, 216, 0.6);
    }
    ...
    .main-content h2 {
        ...
        margin-bottom: 15px;
        padding-top: 20px;
        border-top: 2px solid rgba(0, 173, 216, 0.2);
    }
    .main-content h3 {
        color: #000;
        font-size: 24px;
    ...
    .code-box {
        ...
        margin: 15px 0;
        box-shadow: 0 4px 6px rgba(0, 0, 0, 0.1), 0 0 20px rgba(0, 173, 216, 0.1);
        position: relative;
        max-width: 100%;
        width: 100%;
        box-sizing: border-box;
    ...
    body.dark-mode .sidebar h3 {
        color: #00add8;
        border-bottom-color: rgba(0, 173, 216, 0.3);
    }
    ...
    body.dark-mode .sidebar a:hover {
        background: linear-gradient(135deg, rgba(0, 173, 216, 0.2) 0%, rgba(0, 173, 216, 0.1) 100%);
        color: #00add8;
    }
...
<div class="tutorial-header">
    <div class="tutorial-header-content">
        ...
        <i class="fab fa-python" style="margin-right: 15px;"></i>
            {{ trans('app.formations.go.title') }}
    </h1>
        <p>{{ trans('app.formations.go.subtitle') }}</p>
        <div style="margin-top: 30px; display: flex; justify-content: center; gap: 15px; flex-wrap: wrap;">
            <button data-favorite 
                    data-favorite-type="formation" 
                    data-favorite-slug="python" 
                    data-favorite-name="{{ trans('app.formations.go.title') }}"
...
<div class="tutorial-content">
    <div class="content-wrapper">
        ...
        <button class="sidebar-toggle-btn" id="sidebarToggle" aria-label="{{ trans('app.formations.go.menu_open') }}">
        ...
        <aside class="sidebar" id="tutorialSidebar">
            <div style="display: flex; align-items: center; justify-content: space-between; margin-bottom: 20px; padding-bottom: 15px; border-bottom: 2px solid rgba(0, 173, 216, 0.2);">
                <h3 style="margin: 0;">{{ trans('app.formations.go.sidebar_title') }}</h3>
                <button class="sidebar-close-btn" id="sidebarClose" style="display: none; background: none; border: none; color: #00add8; font-size: 24px; cursor: pointer; padding: 5px; width: 35px; height: 35px; border-radius: 50%; transition: all 0.3s ease;" aria-label="{{ trans('app.formations.go.menu_close') }}">
            ...
            @endphp
            @foreach(trans('app.formations.go.sidebar_menu') as $index => $menuItem)
        ...
        <main class="main-content">
            <h1 id="intro">{{ trans('app.formations.go.intro_title') }}</h1>
            <p>{!! trans('app.formations.go.intro_text') !!}</p>

            <h3>{{ trans('app.formations.go.what_is_title') }}</h3>
            <p>{!! trans('app.formations.go.what_is_text') !!}</p>

            <div class="example-box">
                <h3 style="color: #000;">{{ trans('app.formations.go.why_popular_title') }}</h3>
                <ol style="line-height: 2; font-size: 16px; margin-left: 20px; color: #000;">
                    @foreach(trans('app.formations.go.why_popular_items') as $item)
                    @php
                        $parts = explode(' - ', $item);
                        $label = $parts[0] ?? '';
                        $description = $parts[1] ?? '';
            ...
            <h3>{{ trans('app.formations.go.why_learn_title') }}</h3>
            <p>{{ trans('app.formations.go.why_learn_text') }}</p>
            <ul style="line-height: 2; font-size: 16px; margin-left: 20px; color: #000;">
                @foreach(trans('app.formations.go.why_learn_items') as $item)
                @php
                    $parts = explode(' - ', $item);
                    $label = $parts[0] ?? '';
                    $description = $parts[1] ?? '';
            ...
            <h3>{{ trans('app.formations.go.prerequisites_title') }}</h3>
            <p>{{ trans('app.formations.go.prerequisites_text') }}</p>
            <ul style="line-height: 2; font-size: 16px; margin-left: 20px; color: #000;">
                @foreach(trans('app.formations.go.prerequisites_items') as $item)
                @php
                    $parts = explode(' - ', $item);
                    $label = $parts[0] ?? '';
                    $description = $parts[1] ?? '';
            ...
            <div class="note-box">
                <p style="color: #000;">{!! trans('app.formations.go.prerequisites_note') !!}</p>
            </div>

            <h3>{{ trans('app.formations.go.use_cases_title') }}</h3>
            <p>{{ trans('app.formations.go.use_cases_text') }}</p>
            <ul style="line-height: 2; font-size: 16px; margin-left: 20px; color: #000;">
                @foreach(trans('app.formations.go.use_cases_items') as $item)
                @php
                    $parts = explode(' - ', $item);
                    $icon = '';
                    $label = '';
            ...
            <h2 id="syntax">{{ trans('app.formations.go.syntax_title') }}</h2>
            <p>{!! trans('app.formations.go.syntax_text') !!}</p>
...
print(f"10 + 5 = {resultat}")</code></pre>
            ...
            <div class="example-box">
                <h3 style="color: #000;">{{ trans('app.formations.go.syntax_points_title') }}</h3>
                <ul style="line-height: 2; font-size: 16px; margin-left: 20px; color: #000;">
                    @foreach(trans('app.formations.go.syntax_points_items') as $item)
                    <li>{!! $item !!}</li>
                    @endforeach
                </ul>
            </div>

            <h3>{{ trans('app.formations.go.syntax_example_title') }}</h3>
            <p>{{ trans('app.formations.go.syntax_example_text') }}</p>
...
print(f"La moyenne est : {moyenne}")</code></pre>
            ...
            <h2 id="variables">{{ trans('app.formations.go.variables_title') }}</h2>
            <p>{!! trans('app.formations.go.variables_text') !!}</p>
...
CONSTANTE = 3.14159     # Convention pour les constantes</code></pre>
            ...
            <div class="example-box">
                <h3 style="color: #000;">{{ trans('app.formations.go.variables_rules_title') }}</h3>
                <ul style="line-height: 2; font-size: 16px; margin-left: 20px; color: #000;">
                    @foreach(trans('app.formations.go.variables_rules_items') as $item)
                    <li>{!! $item !!}</li>
                    @endforeach
                </ul>
            </div>

            <h2 id="datatypes">{{ trans('app.formations.go.datatypes_title') }}</h2>
            <p>{{ trans('app.formations.go.datatypes_text') }}</p>
...
prix_float = float("19.99")       # Convertir en décimal</code></pre>
            ...
            <div class="example-box">
                <h3 style="color: #000;">{{ trans('app.formations.go.datatypes_list_title') }}</h3>
                <ul style="line-height: 2; font-size: 16px; margin-left: 20px; color: #000;">
                    @foreach(trans('app.formations.go.datatypes_list_items') as $item)
                    @php
                        $parts = explode(' - ', $item);
                        $label = $parts[0] ?? '';
                        $description = $parts[1] ?? '';
            ...
            <h2 id="operators">{{ trans('app.formations.go.operators_title') }}</h2>
            <p>{{ trans('app.formations.go.operators_text') }}</p>
...
print(liste1 == liste2)    # True (valeurs égales)</code></pre>
            ...
            <h2 id="conditions">{{ trans('app.formations.go.conditions_title') }}</h2>
            <p>{!! trans('app.formations.go.conditions_text') !!}</p>
...
if age >= 18 and permis:
    ...
    print("Vous ne pouvez pas conduire")</code></pre>
            ...
            <h2 id="loops">{{ trans('app.formations.go.loops_title') }}</h2>
            <p>{!! trans('app.formations.go.loops_text') !!}</p>
...
for i in range(5):
    ...
    print("Boucle terminée")  # Exécuté si la boucle se termine normalement</code></pre>
            ...
            <h2 id="functions">{{ trans('app.formations.go.functions_title') }}</h2>
            <p>{!! trans('app.formations.go.functions_text') !!}</p>
...
print(carres)  # [1, 4, 9, 16, 25]</code></pre>
            ...
            <h2 id="lists">{{ trans('app.formations.go.lists_title') }}</h2>
            <p>{{ trans('app.formations.go.lists_text') }}</p>
...
personne.pop("ville")           # Supprimer et retourner la valeur</code></pre>
            ...
            <h2 id="modules">{{ trans('app.formations.go.modules_title') }}</h2>
            <p>{{ trans('app.formations.go.modules_text') }}</p>
...
# print(mon_module.ma_fonction())</code></pre>
            ...
            <h2 id="oop">{{ trans('app.formations.go.oop_title') }}</h2>
            <p>{{ trans('app.formations.go.oop_text') }}</p>
...
print(etudiant.etudier())       # Méthode spécifique</code></pre>
            ...
            <h2 id="files">{{ trans('app.formations.go.files_title') }}</h2>
            <p>{!! trans('app.formations.go.files_text') !!}</p>
...
try:
    ...
    print(f"Erreur : {e}")</code></pre>
            ...
            <div class="note-box">
                <p style="color: #000;">{!! trans('app.formations.go.files_note') !!}</p>
            </div>

            <h2>{{ trans('app.formations.go.next_steps_title') }}</h2>
            <p>{{ trans('app.formations.go.next_steps_text') }}</p>
            
            <div class="example-box" style="background-color: #d4edda; border-left-color: #28a745;">
                <h3 style="color: #000;">{{ trans('app.formations.go.next_steps_learned_title') }}</h3>
                <ul style="margin-left: 20px; line-height: 2; color: #000;">
                    @foreach(trans('app.formations.go.next_steps_learned_items') as $item)
                    <li>{{ $item }}</li>
                    @endforeach
                </ul>
            </div>
            ...
            <div class="example-box">
                <h3 style="color: #000;">{{ trans('app.formations.go.next_steps_further_title') }}</h3>
                <ul style="margin-left: 20px; line-height: 2; color: #000;">
                    @foreach(trans('app.formations.go.next_steps_further_items') as $item)
                    @php
                        $parts = explode(' - ', $item);
                        $icon = '';
                        $label = '';
            ...
            <div class="nav-buttons">
                <a href="{{ route('formations.ia') }}" class="nav-btn">{{ trans('app.formations.go.nav_previous') }}</a>
                <a href="{{ route('exercices') }}" class="nav-btn">{{ trans('app.formations.go.nav_next') }}</a>
            </div>
        </main>
    </div>
</div>
            ...
            try {
                if (isMobile()) {
                    ...
                    if (toggleDisplay === 'none' || toggleDisplay === '') {
                        sidebarToggle.style.cssText = 'display: flex !important; visibility: visible !important; opacity: 1 !important; position: fixed !important; bottom: 20px !important; left: 20px !important; width: 60px !important; height: 60px !important; background: linear-gradient(135deg, #00add8, #2d5f8a) !important; border: none !important; border-radius: 50% !important; color: white !important; font-size: 24px !important; cursor: pointer !important; z-index: 10000 !important; box-shadow: 0 8px 25px rgba(0, 173, 216, 0.4) !important; align-items: center !important; justify-content: center !important;';
                    }
