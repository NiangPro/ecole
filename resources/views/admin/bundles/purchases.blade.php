...
@section('title', 'Achats de packs')

@section('content')
<div class="epreuves-admin">
    <div class="mb-8">
        <h3 class="text-3xl font-bold mb-2">Achats de packs</h3>
        <p class="text-gray-400">Validez les paiements Wave pour livrer les packs (e-mail avec archive .zip).</p>
    </div>
        ...
        <table class="w-full text-left">
            <thead>
                <tr class="border-b border-cyan-500/30">
                    ...
                    <th class="pb-3 pr-4 font-semibold">Pack</th>
            ...
            <tbody>
                ...
                <tr class="border-b border-gray-700/50 hover:bg-gray-800/30 transition">
                    ...
                    <td class="py-3 pr-4">
                        <span class="text-cyan-400 font-medium">{{ \Illuminate\Support\Str::limit($p->bundle?->name ?? '—', 45) }}</span>
                    </td>
                    ...
                    <td class="py-3 pr-4 text-right whitespace-nowrap">
                        @if($p->status === 'pending')
                            <form action="{{ route('admin.bundles.purchases.approve', $p->id) }}" method="POST" class="inline" onsubmit="return confirm('Confirmer la réception du paiement Wave et livrer le pack ?');">
                                @csrf
                            ...
                            <form action="{{ route('admin.bundles.purchases.cancel', $p->id) }}" method="POST" class="inline" onsubmit="return confirm('Annuler cet achat ?');">
                                @csrf
                        ...
                        @elseif($p->status === 'completed' && $p->download_token)
                            @php
                                $downloadLink = route('bundles.payment.download', ['token' => $p->download_token]);
                                $shareText = "Bonjour " . ($p->customer_name ?? '') . ", voici votre pack : " . ($p->bundle?->name ?? '') . "\n\nLien de téléchargement (valable 30 jours) :\n" . $downloadLink;
                            @endphp
                            @if($p->customer_email)
                                <a href="mailto:{{ $p->customer_email }}?subject={{ rawurlencode('Votre pack - ' . ($p->bundle?->name ?? '')) }}&body={{ rawurlencode($shareText) }}" class="inline-flex items-center px-3 py-1.5 bg-blue-500/20 hover:bg-blue-500/30 text-blue-400 rounded text-sm transition mr-1">
                        ...
                        @elseif($p->status === 'failed' && $p->customer_phone)
                            @php
                                $waNum = preg_replace('/[^0-9]/', '', ($p->country_code ?? '') . $p->customer_phone);
                                $bundleLink = $p->bundle?->slug ? route('bundles.show', $p->bundle->slug) : null;
                                $relanceText = "Bonjour " . ($p->customer_name ?? '') . ",\n\n"
                                    . "Nous avons remarqué que votre paiement pour le pack \"" . ($p->bundle?->name ?? '') . "\" n'a pas abouti.\n\n"
                                    . "Souhaitez-vous qu'on vous accompagne pour finaliser votre achat ? C'est rapide et sécurisé."
                                    . ($bundleLink ? "\n\n" . $bundleLink : "");
                            @endphp
                ...
                @empty
                <tr><td colspan="6" class="py-8 text-center text-gray-400">Aucun achat de pack pour le moment.</td></tr>
                @endforelse
            </tbody>
        </table>
    </div>
