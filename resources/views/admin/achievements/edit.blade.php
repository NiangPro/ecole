...
<div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-8">
    <div>
        <h3 class="text-3xl font-bold mb-2">Modifier Réalisation</h3>
        <p class="text-gray-400">Mettez à jour les informations de la réalisation</p>
    </div>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
            <div class="space-y-6">
                ...
                <div>
                    ...
                    <textarea name="description" rows="4" class="input-admin" placeholder="Description de la réalisation">{{ old('description', $achievement->description) }}</textarea>
                </div>
                ...
                <div>
                    ...
                    <input type="url" name="link_url" value="{{ old('link_url', $achievement->link_url) }}"
            ...
            <div class="space-y-6">
                <div>
                    ...
                    <select name="image_type" id="achievementImageType" class="input-admin mb-3">
                        <option value="internal" {{ old('image_type', $achievement->image_type ?? 'internal') === 'internal' ? 'selected' : '' }}>Interne (upload)</option>
                        <option value="external" {{ old('image_type', $achievement->image_type ?? '') === 'external' ? 'selected' : '' }}>Externe (URL)</option>
                    </select>
                    <div id="achievementInternalImage" style="display: {{ old('image_type', $achievement->image_type ?? 'internal') === 'internal' ? 'block' : 'none' }};">
                        <input type="file" name="image_file" accept="image/*" class="input-admin">
                        @if($achievement->image_type === 'internal' && $achievement->image)
                            <p class="text-gray-400 text-sm mt-1">
                                Image actuelle: <a href="{{ \Illuminate\Support\Facades\Storage::url($achievement->image) }}" target="_blank" class="text-cyan-400 hover:underline">{{ basename($achievement->image) }}</a>
                            </p>
                        @else
                            <p class="text-gray-400 text-sm mt-1">Formats acceptés: JPG, PNG, GIF</p>
                        @endif
                    </div>
                    <div id="achievementExternalImage" style="display: {{ old('image_type', $achievement->image_type ?? '') === 'external' ? 'block' : 'none' }};">
                        <input type="url" name="image_url" id="achievementImageUrl" value="{{ old('image', $achievement->image ?? '') }}"
                ...
                <div>
                    <label class="flex items-center gap-3 cursor-pointer">
                        <input type="checkbox" name="is_visible" value="1" {{ old('is_visible', $achievement->is_visible) ? 'checked' : '' }}
