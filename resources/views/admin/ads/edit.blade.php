...
<div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-6">
    <div>
        <h3 class="text-3xl font-bold mb-2">Modifier Publicité</h3>
        <p class="text-gray-400">Modifiez les paramètres de la publicité</p>
    </div>
        ...
        <div class="lg:col-span-2 space-y-6">
            <div class="content-section">
                ...
                <input type="text" name="name" value="{{ old('name', $ad->name) }}" required
                       class="input-admin" placeholder="Ex: Bannière Sidebar 300x250">
                @error('name')
            ...
            <div class="content-section">
                ...
                <textarea name="description" rows="3" class="input-admin" placeholder="Description optionnelle de la publicité">{{ old('description', $ad->description) }}</textarea>
            </div>
                ...
                <div class="space-y-4">
                    <div>
                        ...
                        <select name="image_type" id="adImageType" class="input-admin" required>
                            <option value="external" {{ old('image_type', $ad->image_type ?? 'external') === 'external' ? 'selected' : '' }}>Externe (URL)</option>
                            <option value="internal" {{ old('image_type', $ad->image_type) === 'internal' ? 'selected' : '' }}>Interne (upload)</option>
                        </select>
                    </div>
                    <div id="adInternalImage" style="display: {{ old('image_type', $ad->image_type ?? 'external') === 'internal' ? 'block' : 'none' }};">
                        ...
                        @if($ad->image_type === 'internal' && $ad->image)
                            <p class="text-gray-400 text-sm mt-1">Image actuelle: <a href="{{ \Illuminate\Support\Facades\Storage::url($ad->image) }}" target="_blank" class="text-cyan-400 hover:underline">{{ basename($ad->image) }}</a></p>
                        @else
                            <p class="text-gray-400 text-sm mt-1">Formats acceptés: JPG, PNG, GIF</p>
                        @endif
                    </div>
                    <div id="adExternalImage" style="display: {{ old('image_type', $ad->image_type ?? 'external') === 'external' ? 'block' : 'none' }};">
                        <label class="block text-cyan-400 mb-2 font-semibold">URL de l'image</label>
                        <input type="url" name="image_url" value="{{ old('image', $ad->image) }}"
                    ...
                    <div>
                        ...
                        <input type="url" name="link_url" value="{{ old('link_url', $ad->link_url) }}" required
                    ...
                    <div id="adImagePreview" class="mt-4 {{ $ad->image ? '' : 'hidden' }}">
                        <img id="adPreviewImg" src="{{ $ad->image ? ($ad->image_type === 'internal' ? \Illuminate\Support\Facades\Storage::url($ad->image) : $ad->image) : '' }}" alt="Aperçu" class="w-full rounded-lg border border-cyan-500/20 max-h-64 object-contain">
                    </div>
                </div>
            </div>
        </div>
            ...
            <div class="content-section">
                ...
                <div class="mb-4">
                    ...
                    <select name="position" id="adPosition" required class="input-admin">
                        ...
                        <option value="content" {{ old('position', $ad->position) === 'content' ? 'selected' : '' }}>Contenu</option>
                        <option value="header" {{ old('position', $ad->position) === 'header' ? 'selected' : '' }}>En-tête</option>
                        <option value="footer" {{ old('position', $ad->position) === 'footer' ? 'selected' : '' }}>Pied de page</option>
                    </select>
                    @error('position')
                        <p class="text-red-400 text-sm mt-1">{{ $message }}</p>
                    @enderror
                ...
                <div class="mb-4">
                    ...
                    <select name="location" id="adLocation" class="input-admin">
                        ...
                        <option value="homepage_after_exercises" {{ old('location', $ad->location) === 'homepage_after_exercises' ? 'selected' : '' }}>Page d'accueil - Après Exercices & Quiz</option>
                        <option value="article_sidebar" {{ old('location', $ad->location) === 'article_sidebar' ? 'selected' : '' }}>Articles - Sidebar</option>
                    </select>
            ...
            <div class="content-section">
                ...
                <div class="space-y-3 text-sm">
                    <div class="flex justify-between">
                        ...
                        <span class="text-white font-semibold">{{ number_format($ad->impressions) }}</span>
                    </div>
                    <div class="flex justify-between">
                        <span class="text-gray-400">Clics:</span>
                        <span class="text-white font-semibold">{{ number_format($ad->clicks) }}</span>
                    </div>
                    @if($ad->impressions > 0)
                    <div class="flex justify-between">
                        <span class="text-gray-400">CTR:</span>
                        <span class="text-white font-semibold">{{ number_format(($ad->clicks / $ad->impressions) * 100, 2) }}%</span>
                    </div>
                    @endif
                </div>
            </div>
    ...
    if (adImageUrl) {
        adImageUrl.addEventListener('input', function() {
            if (this.value) {
                ...
                adImagePreview.classList.remove('hidden');
            } else if (adImageType.value === 'external') {
                adImagePreview.classList.add('hidden');
                adPreviewImg.src = '';
            }
        });
