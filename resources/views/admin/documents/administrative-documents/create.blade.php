...
@section('title', 'Nouvelle fiche document administratif')
...
<div class="max-w-5xl mx-auto">
    <div class="mb-6 flex items-center justify-between gap-4">
        ...
        <div class="text-sm text-gray-400 hidden sm:block">
            <span class="px-2 py-1 rounded-full border border-cyan-500/40 bg-cyan-500/10 text-cyan-300">
                Mode création ultra moderne
            </span>
        </div>
    </div>
    <div class="admin-adm-doc-form-card p-6 rounded-xl bg-gray-800/80 border border-cyan-500/30 shadow-xl">
        <div class="flex items-start justify-between gap-4 mb-6">
            <div>
                <h1 class="text-2xl font-bold text-cyan-400">Nouvelle fiche document administratif</h1>
                <p class="mt-1 text-sm text-gray-400">Renseigne toutes les informations nécessaires pour générer une fiche claire sur les papiers administratifs.</p>
            </div>
        </div>
        <form action="{{ route('admin.documents.administrative-documents.store') }}" method="POST" enctype="multipart/form-data" class="space-y-6">
            @csrf
            @include('admin.documents.administrative-documents._form', ['document' => null, 'categories' => $categories])
