...
@php
    $selectedDocIds = old('document_ids', $bundle->items->where('item_type', \App\Models\Document::class)->pluck('item_id')->toArray());
    $selectedEpreuveIds = old('epreuve_ids', $bundle->items->where('item_type', \App\Models\Epreuve::class)->pluck('item_id')->toArray());
@endphp
...
<div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-8">
    <div>
        <h3 class="text-3xl font-bold mb-2">Modifier le Pack</h3>
        <p class="text-gray-400">{{ $bundle->name }}</p>
    </div>
...
<div class="content-section">
    <form action="{{ route('admin.documents.bundles.update', $bundle->id) }}" method="POST" enctype="multipart/form-data">
        @csrf
        @method('PUT')
        ...
        <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
            <div class="space-y-6">
                <div>
                    ...
                    <input type="text" name="name" value="{{ old('name', $bundle->name) }}" required class="input-admin">
                    @error('name')<p class="text-red-400 text-sm mt-1">{{ $message }}</p>@enderror
                </div>

                <div>
                    <label class="block text-cyan-400 mb-2 font-semibold">Description</label>
                    <textarea name="description" rows="4" class="input-admin">{{ old('description', $bundle->description) }}</textarea>
                ...
                <div class="grid grid-cols-2 gap-4">
                    <div>
                        ...
                        <input type="number" name="price" value="{{ old('price', $bundle->price) }}" min="0" step="50" required class="input-admin">
                    ...
                    <div>
                        ...
                        <input type="number" name="discount_price" value="{{ old('discount_price', $bundle->discount_price) }}" min="0" step="50" class="input-admin" placeholder="Optionnel">
                ...
                <div>
                    ...
                    <p class="text-gray-400 text-sm mb-2">
                        ...
                        @if($bundle->cover_type === 'internal')
                            <a href="{{ \Illuminate\Support\Facades\URL::signedRoute('bundle.cover.signed', ['id' => $bundle->id]) }}" target="_blank" class="text-cyan-400 underline">voir l'image</a>
                        @else
                            <a href="{{ $bundle->cover_image }}" target="_blank" class="text-cyan-400 underline">voir l'image</a>
                        @endif
                    </p>
                    @endif
                    <select name="cover_type" id="coverType" class="input-admin mb-3">
                        <option value="internal" {{ old('cover_type', $bundle->cover_type ?? 'internal') === 'internal' ? 'selected' : '' }}>Interne (upload)</option>
                        <option value="external" {{ old('cover_type', $bundle->cover_type) === 'external' ? 'selected' : '' }}>Externe (URL)</option>
                    </select>
                    <div id="coverInternal" style="display: {{ old('cover_type', $bundle->cover_type ?? 'internal') === 'internal' ? 'block' : 'none' }};">
                        <input type="file" name="cover_image_file" accept="image/*" class="input-admin">
                        <p class="text-gray-400 text-sm mt-1">Laisser vide pour conserver l'image actuelle</p>
                        @error('cover_image_file')<p class="text-red-400 text-sm mt-1">{{ $message }}</p>@enderror
                    </div>
                    <div id="coverExternal" style="display: {{ old('cover_type', $bundle->cover_type) === 'external' ? 'block' : 'none' }};">
                        <input type="url" name="cover_image_url" value="{{ old('cover_image_url', $bundle->cover_type === 'external' ? $bundle->cover_image : '') }}" class="input-admin" placeholder="https://exemple.com/image.jpg">
            ...
            <div class="space-y-5">
                ...
                <div>
                    ...
                    <div class="doc-picker" id="doc-picker-list">
                        ...
                        <label class="doc-picker-item" data-doc-title="{{ \Illuminate\Support\Str::lower($document->title) }}">
                            <input type="checkbox" name="document_ids[]" value="{{ $document->id }}"
                                   {{ in_array($document->id, $selectedDocIds) ? 'checked' : '' }}>
                ...
                <div>
                    ...
                    <div class="doc-picker" id="epreuve-picker-list">
                        ...
                        <label class="doc-picker-item" data-doc-title="{{ \Illuminate\Support\Str::lower($epreuve->title) }}">
                            <input type="checkbox" name="epreuve_ids[]" value="{{ $epreuve->id }}"
                                   {{ in_array($epreuve->id, $selectedEpreuveIds) ? 'checked' : '' }}>
