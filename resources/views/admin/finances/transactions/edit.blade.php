...
@section('title', 'Modifier la transaction - Admin')
...
<div class="txn-page">
    ...
    <div class="txn-hero">
        <div class="txn-hero-icon">✏️</div>
        <div class="txn-hero-text">
            <h1>Modifier la transaction</h1>
            <p>« {{ $transaction->label }} » — {{ number_format($transaction->amount, 0, ',', ' ') }} {{ $transaction->currency }} du {{ $transaction->transaction_date->format('d/m/Y') }}</p>
        </div>
    ...
    <div class="txn-layout">
        <form method="POST" action="{{ route('admin.finances.transactions.update', $transaction) }}" class="txn-form-card" id="txnForm">
            @csrf
            @method('PUT')
                ...
                <select name="finance_category_id" id="categorySelect" class="txn-input" required>
                    ...
                    <optgroup label="{{ $type === 'income' ? '↑ Revenus' : '↓ Dépenses' }}">
                        ...
                        <option value="{{ $cat->id }}" data-type="{{ $cat->type }}" data-icon="{{ $cat->icon }}" data-name="{{ $cat->name }}"
                            {{ old('finance_category_id', $transaction->finance_category_id) == $cat->id ? 'selected' : '' }}>
                            {{ $cat->icon }} {{ $cat->name }}
                        </option>
                        @endforeach
                    </optgroup>
            ...
            <div class="txn-row">
                ...
                <div class="txn-field txn-currency">
                    ...
                    <select name="currency" id="currencySelect" class="txn-input">
                        <option value="XOF" {{ old('currency', $transaction->currency) == 'XOF' ? 'selected' : '' }}>XOF</option>
            ...
            <div class="txn-field" style="margin-bottom: 0;">
                ...
                <textarea name="notes" id="notesInput" class="txn-input" rows="3">{{ old('notes', $transaction->notes) }}</textarea>
            </div>

            <div class="txn-actions">
                <button type="submit" class="txn-btn-submit"><i class="fas fa-check"></i> Enregistrer les modifications</button>
        ...
        <aside class="txn-preview" id="txnPreview">
            ...
            <div class="txn-preview-icon" id="previewIcon">{{ $transaction->category->icon }}</div>
            <div class="txn-preview-amount {{ $transaction->type === 'income' ? 'is-income' : 'is-expense' }}" id="previewAmount">
                {{ $transaction->type === 'income' ? '+' : '−' }}{{ number_format($transaction->amount, 0, ',', ' ') }} {{ $transaction->currency }}
            </div>
            <div class="txn-preview-converted" id="previewConverted">
                @if($transaction->currency !== 'XOF')
                ≈ {{ number_format($transaction->amount_xof, 0, ',', ' ') }} XOF au taux actuel
                @endif
            </div>
            <div class="txn-preview-meta">
                <div class="txn-preview-row"><span>Type</span><span id="previewType">{{ $transaction->type === 'income' ? 'Revenu' : 'Dépense' }}</span></div>
                <div class="txn-preview-row"><span>Catégorie</span><span id="previewCategory">{{ $transaction->category->icon }} {{ $transaction->category->name }}</span></div>
                <div class="txn-preview-row"><span>Date</span><span id="previewDate">{{ $transaction->transaction_date->translatedFormat('d F Y') }}</span></div>
            </div>
